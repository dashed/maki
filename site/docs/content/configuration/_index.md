...
| `always_thinking` | bool \| string | `false` | Start every session with extended thinking (true/"adaptive", "off", an effort level ("minimal" to "max"), or a token budget). Or press `d` in `/thinking` to remember a choice without editing this file; setting it here wins over that |
